# harbour_master.gd — Tobias Sandcrest, Harbour Master of Sandcrest Landing (the dock). Built on VendorNPC: right-click (or hail)
# opens his shop, which sells the Bastion ferry token. The token does nothing yet — the Bastion ship is away until Thallia's
# Bastion exists — and he says so when the shop opens. He also talks on his own: every few minutes a line of ambient chatter
# to anyone nearby (at night, sometimes a secret line), and his greeting mentions Sahren (the traveling merchant) depending on
# where Sahren is right now — so the world hints at the schedule without any UI. All his lines: Data/harbour_master.json.
# Multiplayer: he is a static scene node on every peer; the SERVER decides the ambient line and relays it (each peer applies
# its own hearing range), same as the guards' banter.
extends VendorNPC
class_name HarbourMaster


const CONFIG_PATH := "res://Data/harbour_master.json"
const HEAR_RANGE := 12.0

@export var model_key: String = "half_elf_male"  # STAND-IN until his Meshy model exists
...
func _ready() -> void:
	...
	super._ready()   # VendorNPC stands him on the dock's deck (it is 1 m up), wherever the scene has him
...
# Said when his shop opens (right-click or hail): a greeting — sometimes a remark about Sahren — then the honest word on
# the ferry, because the token he sells does nothing yet.
func greet_player(_player_name: String) -> void:
	_face_player()
	var line := _pick(_merchant_category()) if _merchant_category() != "" and randf() < 0.4 else _pick("greeting")
	say_local(line)
	say_local(_pick("ferry_away"))
...
func say_local(line: String) -> void:
	...
	GameLog.log_general("[color=#cccc88]%s says%s, \"%s\"[/color]" % [get_vendor_display_name(), spoken[0], NPCConversation.format(spoken[1])])
...
# Lines a topic pulls from him: his own line pools by name, or a remark about Sahren that matches where Sahren is.
func dynamic_lines(name: String) -> Array:
	if name == "merchant_remark":
		var category := _merchant_category()
		var pool = _config.get(category if category != "" else "merchant_none", [])
