...
# NDA.txt (the playtest confidentiality terms) and TESTER_GUIDE.html (project root) are copied next to the game in each full build.
#
...
if [ "$DO_CLIENT" = 1 ]; then
	for plat in $PLATFORM_LIST; do
		...
		if [ "$NEW_BASE" = 1 ]; then
			...
			cp NDA.txt TESTER_GUIDE.html "$P_DIR/"  # the playtest NDA and tester guide ship next to the game in every full build
